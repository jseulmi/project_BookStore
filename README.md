...
👨‍💻 담당 역할
- 🗂 **DB 설계** : 도서(Book) 테이블 구조 설계, 도서–리뷰 간 관계 정의
- 📚 **도서 조회 기능** : 전체 도서 목록 조회 및 도서 상세 조회 기능 구현, 카테고리별 도서 조회 API 설계, 페이징 처리 적용으로 대량 데이터 조회 성능 개선-
- ✍️ **도서 리뷰 기능**: 도서 리뷰 등록 기능 구현, 사용자 권한에 따른 리뷰 작성 제한 로직 설계 및 구현
- 🔐 **리뷰 작성 제한 정책**
