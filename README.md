...
온라인 북스토어는 **도서 검색, 장바구니, 결제, 주문 관리** 기능을 제공하는 웹 기반 도서 쇼핑몰 프로젝트입니다.
...
<summary><strong>📚 도서 기능</strong></summary>
  ...
  - ✍️ **도서 리뷰 기능**
    - 도서 리뷰 등록
    - <details>
      <summary><strong>구매 이력 기반 리뷰 작성 제한</strong></summary>
      미구매 사용자: 리뷰 작성 영역 미노출 / 구매 사용자: 도서당 리뷰 1회 제한 / 리뷰 작성 완료 시 안내 메시지 출력
...
사용자는 키워드 및 장르 조건을 활용해 원하는 도서를 조회할 수 있으며,
검색 조건 유무에 따라 전체 조회와 조건 검색을 분기 처리하여 불필요한 쿼리 실행을 방지했습니다.
  
- 🔍 도서 검색 및 필터 조회
  - 검색어(q)와 장르 ID(genre_id)를 Request Parameter로 전달받아 처리합니다.
  - 검색어와 장르가 모두 없는 경우 → 전체 도서 목록 조회합니다.
  - 검색어 또는 장르가 존재하는 경우 → 조건 검색 쿼리 실행합니다.
  - 키워드 + 장르를 동시에 적용한 복합 조건 검색 지원합니다.
  - MyBatis Map 파라미터를 활용해 동적 SQL 처리합니다.
</details>
...
- 📝 도서 리뷰 조회
  - 도서 상세 페이지 진입 시 해당 도서에 등록된 리뷰 목록 조회합니다.
  - 도서 ID를 기준으로 리뷰 데이터를 조회하여 사용자에게 제공합니다.
 
- 🔐 리뷰 작성 권한 검증 로직
  - 리뷰 작성 가능 여부는 로그인 여부, 구매 이력, 리뷰 작성 이력을 기준으로 판단합니다.
  - 1️⃣ 로그인 사용자 확인 : 세션(HttpSession)을 통해 로그인 사용자 ID 조회, 비로그인 사용자는 리뷰 작성 불가합니다.
  - 2️⃣ 도서 구매 이력 확인 : 주문 목록과 주문 상세 데이터를 조회, 해당 도서를 구매한 이력이 있는지 서버 단에서 검증합니다.
  - 3️⃣ 리뷰 중복 작성 방지 : 사용자 ID + 도서 ID 기준으로 리뷰 작성 여부 조회 , 이미 리뷰를 작성한 경우, 추가 작성 제한합니다.

- 🖥 화면 제어 방식
  - 구매 이력이 없는 사용자 → 리뷰 작성 영역 미노출합니다.
  - 구매 이력이 있고 리뷰를 작성하지 않은 사용자 → 리뷰 작성 영역 노출합니다.
  - 이미 리뷰를 작성한 사용자 → 리뷰 작성 영역 대신 “이미 리뷰를 작성한 도서입니다” 안내 메시지 표시합니다.
 
👉 UI 제어는 서버에서 전달한 상태 값(hasPurchased, hasReviewed)을 기준으로 처리합니다.

- 구현 특징
  - 서버 단에서 리뷰 작성 가능 여부를 판단하여 보안 및 데이터 무결성 확보합니다.
  - 구매 이력 기반 리뷰 작성 제한으로 신뢰도 있는 리뷰 시스템 구현합니다.
  - 도서–주문–리뷰 간 관계를 활용한 확장 가능한 구조 설계합니다.
