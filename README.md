...
- 🔍 도서 검색 및 필터 조회
  - 검색어(q)와 장르 ID(genre_id)를 Request Parameter로 전달받아 처리
  - 검색어와 장르가 모두 없는 경우 → 전체 도서 목록 조회
...
- 🔐 리뷰 작성 권한 검증 로직
  ...
  - 1️⃣ 로그인 사용자 확인 : 세션(HttpSession)을 통해 로그인 사용자 ID 조회, 비로그인 사용자는 리뷰 작성 불가
  - 2️⃣ 도서 구매 이력 확인 : 주문 목록과 주문 상세 데이터를 조회, 해당 도서를 구매한 이력이 있는지 서버 단에서 검증
  - 3️⃣ 리뷰 중복 작성 방지 : 사용자 ID + 도서 ID 기준으로 리뷰 작성 여부 조회 , 이미 리뷰를 작성한 경우, 추가 작성 제한

- 🖥 화면 제어 방식
  - 구매 이력이 없는 사용자 → 리뷰 작성 영역 미노출
  - 구매 이력이 있고 리뷰를 작성하지 않은 사용자 → 리뷰 작성 영역 노출
  - 이미 리뷰를 작성한 사용자 → 리뷰 작성 영역 대신 “이미 리뷰를 작성한 도서입니다” 안내 메시지 표시
 
👉 UI 제어는 서버에서 전달한 상태 값(hasPurchased, hasReviewed)을 기준으로 처리

- 구현 특징
  - 서버 단에서 리뷰 작성 가능 여부를 판단하여 보안 및 데이터 무결성 확보
  - 구매 이력 기반 리뷰 작성 제한으로 신뢰도 있는 리뷰 시스템 구현
  - 도서–주문–리뷰 간 관계를 활용한 확장 가능한 구조 설계
